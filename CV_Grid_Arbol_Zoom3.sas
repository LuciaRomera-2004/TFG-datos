...
* VALIDACI”N CRUZADA MACRO;

* PASO 0. ASIGNAR FOLD A CADA OBSERVACI”N DE TRAIN;
...
* PASO 1. TABLA VACÕçA PARA GUARDAR RESULTADOS;
...
%macro cv_hpsplit_fold(d=, l=, f=);
    ...
    * Entrenar ·rbol y exportar cÛdigo;
    ...
    * Aplicar ·rbol al fold de validaciÛn;
    data score_val;
        set cv_val;
...
* PASO 3. MACRO GRID llama al macro de fold para cada
           combinaciÛn de hiperpar·metros;
%macro grid_cv_arbol;

    %let leaves = 15 25 26 27 28 29 30 40;
    %let n_l = %sysfunc(countw(&leaves));

    %do j = 1 %to &n_l;
        %let l = %scan(&leaves, &j);
        %do f = 1 %to 5;
            %cv_hpsplit_fold(d=8, l=&l, f=&f);
        %end;
    %end;
...
* PASO 4. TABLA RESUMEN (media y desv.tÌpica por arquitectura);
proc means data=resultados_cv_arbol mean std min max;
    ...
    title "Resumen CV k=5 ¡Årbol de ClasificaciÛn (Zoom 3)";
run;
...
proc print data=ranking_arbol noobs;
    ...
    title "Ranking arquitecturas CV k=5 (ordenado por Se media)";
run;
